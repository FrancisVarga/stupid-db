import { type TableInfo, type ColumnInfo, listTables, getColumns } from "./introspect";
...
export async function generateOpenAPISpec(
  ...
  for (const table of tables) {
    ...
    const columns = await getColumns(sql, table.name);
    ...
    const listPath = `/api/v1/${dbName}/${table.name}`;
    paths[listPath] = {
      get: buildListOp(table, schemaName),
      post: buildCreateOp(table, inputSchemaName),
      patch: buildBatchUpdateOp(table, inputSchemaName),
      delete: buildBatchDeleteOp(table),
    };

    // ── Single record path: /api/v1/{db}/{table}/{id}
    if (pkFields.length > 0) {
      const singlePath = `/api/v1/${dbName}/${table.name}/{id}`;
      paths[singlePath] = {
        get: buildGetOp(table, schemaName),
        put: buildUpdateOp(table, inputSchemaName),
        delete: buildDeleteOp(table),
      };
    }
  }
...
function buildListOp(table: TableInfo, schemaName: string): Record<string, unknown> {
  return {
...
function buildCreateOp(table: TableInfo, inputSchemaName: string): Record<string, unknown> {
  return {
...
function buildBatchUpdateOp(table: TableInfo, inputSchemaName: string): Record<string, unknown> {
  return {
    summary: `Batch update ${table.name} records`,
    operationId: `batch_update_${table.name}`,
    tags: [table.name],
    requestBody: {
      required: true,
      content: {
...
function buildBatchDeleteOp(table: TableInfo): Record<string, unknown> {
  return {
    summary: `Batch delete ${table.name} records`,
    operationId: `batch_delete_${table.name}`,
    tags: [table.name],
    requestBody: {
      required: true,
      content: {
...
function buildGetOp(table: TableInfo, schemaName: string): Record<string, unknown> {
  return {
    summary: `Get single ${table.name} record`,
...
function buildUpdateOp(table: TableInfo, inputSchemaName: string): Record<string, unknown> {
  return {
    summary: `Update single ${table.name} record`,
...
function buildDeleteOp(table: TableInfo): Record<string, unknown> {
  return {
    summary: `Delete single ${table.name} record`,
