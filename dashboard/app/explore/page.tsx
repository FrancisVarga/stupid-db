  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-slate-600 text-sm animate-pulse">Loading explorer...</div>
      </div>
    }>
      <ExploreContent />
    </Suspense>
...
function ExploreContent() {
  ...
  const initialTab = (searchParams.get("tab") as VizTab) || "graph";
  ...
  const [activeTab, setActiveTab] = useState<VizTab>(initialTab);
  // error state removed — explore page shows offline message in chat instead
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setMessages([makeMsg("system", welcome, { suggestions })]);
        ...
        setMessages([makeMsg("system",
          "Server offline. The dashboard is running in offline mode — visualizations will load when the server becomes available.\n\nStart the server: `stupid-server serve`",
          { suggestions },
        )]);
      }
    };
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/queue" className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium tracking-wide hover:opacity-80 transition-opacity" style={{ background: 'rgba(0, 240, 255, 0.08)', border: '1px solid rgba(0, 240, 255, 0.2)', color: '#00f0ff' }}>
            <span className="w-1.5 h-1.5 rounded-full" style={{ background: Object.values(queueStatus?.queues ?? {}).some((q) => q.connected) ? '#00ff88' : '#64748b' }} />
            Queue
          </Link>
